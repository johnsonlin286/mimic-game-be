...
exports.default = calculateVoteResults;
function calculateVoteResults(players, usePassivePowers = null) {
    if (!players?.length) {
        return {
            success: false,
    ...
    const superpowerName = (p) => p.superpower?.name ?? null;
    const selectedPassive = usePassivePowers && usePassivePowers.isActive ? usePassivePowers : null;
    ...
    for (const p of players) {
        ...
        let effective = raw;
        const isSelectedBriber = selectedPassive?.powerName === "briber"
    ...
    let chiefTriggeredEmail = null;
    if (isTie && selectedPassive?.powerName === "chief") {
        const chief = players.find(p => p.isAlive
        ...
        if (chief) {
            ...
            const chiefVotedForEmail = players.find(p => p.isAlive && p.voters.some(v => v.playerEmail === chief.playerEmail))?.playerEmail ?? null;
            if (chiefVotedForEmail) {
                const boostedVotes = new Map(effectiveVotes);
                boostedVotes.set(chiefVotedForEmail, (boostedVotes.get(chiefVotedForEmail) ?? 0) + 1);
    ...
    const powersTriggered = new Set();
    if (briberTriggeredEmail)
        powersTriggered.add(briberTriggeredEmail);
    if (chiefTriggeredEmail)
        powersTriggered.add(chiefTriggeredEmail);
    const playersWithPowerUpdates = powersTriggered.size > 0
        ? players.map(p => powersTriggered.has(p.playerEmail)
    ...
    const newPlayers = playersWithPowerUpdates.map(p => p.isAlive && p.playerEmail === topEmail ? { ...p, isAlive: false } : p);
    ...
    const isSelectedSaboteur = !!selectedPassive
    ...
    if (oppositionWins) {
        const oppositionMessage = minorityCount > 0
...
//# sourceMappingURL=voteScript.js.map