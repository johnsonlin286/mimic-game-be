...
function calculateVoteResults(players) {
    ...
    let mimicCount = 0;
    let voidCount = 0;
    const newPlayers = new Array(players.length);
    for (let i = 0; i < players.length; i += 1) {
        const p = players[i];
        const next = p.isAlive && p.playerEmail === topEmail ? { ...p, isAlive: false } : p;
        newPlayers[i] = next;
        if (!next.isAlive)
            continue;
        if (next.gameRole === "original")
            originalCount += 1;
        else if (next.gameRole === "mimic")
            mimicCount += 1;
        else if (next.gameRole === "void")
            voidCount += 1;
    }
