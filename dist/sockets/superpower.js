...
exports.default = registerSuperpowerHandlers;
const algorithmScript_1 = require("../algorithmScript");
const guards_1 = require("./guards");
const serializers_1 = require("./serializers");
function registerSuperpowerHandlers(io, socket) {
    const useSuperpower = (payload) => {
        const room = (0, guards_1.findRoom)(socket, payload.roomId, "use-superpower-failed");
        if (!room)
            return;
        const player = (0, guards_1.findGamePlayer)(socket, room, payload.playerEmail, "use-superpower-failed");
        if (!player)
            return;
        ...
        let players = room.gameData?.players?.filter(p => p.isAlive && p.playerEmail !== payload.playerEmail) ?? [];
        switch (payload.powerName) {
            case "interrogator":
                socket.emit("use-superpower-interrogator", {
                    success: true,
                    message: "You used the interrogator specialist",
                    data: players.map(p => (0, serializers_1.maskedPlayer)(p)),
                });
            ...
            case "wiretapper":
                ...
                const playersWords = players.map(p => ({
                    ...
                    playerName: p.playerName,
                    playerEmail: p.playerEmail,
                    gameWord: p.gameRole === "blind" ? "NO SIGNAL" : (0, algorithmScript_1.maskWordWithHint)(p.gameWord),
                }));
    ...
    const usePassivePower = (payload) => {
        const room = (0, guards_1.findRoom)(socket, payload.roomId, "use-passive-power-failed");
        if (!room)
            return;
        const player = (0, guards_1.findGamePlayer)(socket, room, payload.playerEmail, "use-passive-power-failed");
        if (!player)
            return;
        if (player.superpower?.name !== payload.powerName) {
    ...
    const interrogatorPickTarget = (payload) => {
        const room = (0, guards_1.findRoom)(socket, payload.roomId, "interrogator-pick-target-failed");
        if (!room)
            return;
        const player = (0, guards_1.findGamePlayer)(socket, room, payload.playerEmail, "interrogator-pick-target-failed");
        if (!player)
            return;
        ...
        const targetPlayer = (0, guards_1.findGamePlayer)(socket, room, payload.targetPlayerEmail, "interrogator-pick-target-failed");
        if (!targetPlayer) {
...
//# sourceMappingURL=superpower.js.map