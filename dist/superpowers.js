...
exports.AGENT = exports.randomSuperpower = exports.superpowers = void 0;
exports.assignSuperpowersForRound = assignSuperpowersForRound;
 ...
 */
function assignSuperpowersForRound(players, numActivePowers, numPassivePowers, previousSuperpowerNames = []) {
    for (const p of players) {
        p.superpower = AGENT;
    }
    const totalSlots = numActivePowers + numPassivePowers;
    if (totalSlots === 0 || players.length === 0)
        return [];
     ...
     */
    function pickNames(pool, needed) {
    ...
    for (const { name } of slots) {
        ...
        if (!def)
            continue;
        const candidates = players.filter(p => p.superpower === AGENT && def.allowedRoles.includes(p.gameRole));
...
const randomSuperpower = (previous) => {
    const remaining = superpowers.filter(power => power.name !== previous);
    return remaining[Math.floor(Math.random() * remaining.length)];
};
exports.randomSuperpower = randomSuperpower;
