...
import { roomBroadcast } from "./serializers";

/** Returns true if `playerEmail` is currently in any room (optionally excluding one). */
function emailExistsInAnyRoom(playerEmail: string, exceptRoomId?: string): boolean {
  for (const room of rooms.values()) {
    if (exceptRoomId && room.roomId === exceptRoomId) continue;
    if (room.roomPlayers.some(p => p.playerEmail === playerEmail)) return true;
  }
  return false;
}
...
function syncLobbyStatus(room: RoomData): void {
  if (room.gameRule.status === "playing") return;
